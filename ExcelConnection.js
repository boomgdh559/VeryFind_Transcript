...
// var workBook = xlsx.readFile("./SIT-IT-Transcript.xlsx");
...
//ar data = xlsx.utils.sheet_to_json(workBook.Sheets[workSheet[0]]);
...
formatterJSON = (data) => {
    ...
    var allSemester = patternSemester.map((pattern) => {
        ...
        patternEachCourse = pattern.match(/\"courseTitle":"(.*?)\"[A-F][+-]?"/g);
